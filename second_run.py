...
class CustomRewardWrapper(Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self.theta_idx = 2  # Pole angle is 3rd element in state (0-based index)
    
    def step(self, action):
        # Get original step result
        obs, reward, terminated, truncated, info = self.env.step(action)
        
        # Custom reward: -|pole angle| (penalize tilt magnitude)
        custom_reward = -abs(obs[self.theta_idx])  # obs[2] = theta in radians
...
if __name__ == "__main__":
    ...
    # Loads the weights
    if os.path.isfile("second_run.weights.h5"):
        agent.load_weights("second_run.weights.h5")
    ...
    for episode in range(num_episodes):
        ...
        if total_reward >= max_reward:
            agent.save_weights("second_run.weights.h5")
    ...
    plt.title('DQN CartPole: Total Reward vs. Episode (200 Episodes)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()
